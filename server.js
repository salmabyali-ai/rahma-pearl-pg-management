...
// =====================================
// SEND WHATSAPP TO ALL STUDENTS
// =====================================

app.post("/admin/whatsapp/send", async (req, res) => {
    ...
    try {
        ...
        for (const student of students) {
            ...
            const text = `Hello ${student.first_name}, ${message}`;

            try {

                await sendWhatsApp(student.phone, text);

            } catch (err) {

                console.log("WhatsApp failed for:", student.phone);
        ...
        res.render("whatsapp_broadcast", {
            message: "WhatsApp broadcast failed."
        });
...
// =====================================
// SEND WHATSAPP TO ONE STUDENT
// =====================================

app.post("/admin/whatsapp/student/:id", async (req, res) => {
    ...
    try {
        ...
        const [rows] = await db.query(
            "SELECT first_name, phone FROM students WHERE id=?",
            [req.params.id]
        );
        ...
        await sendWhatsApp(
            student.phone,
            `Hello ${student.first_name}, ${message}`
        );
...
cron.schedule("0 9 * * *", async () => {
    ...
    try {
        ...
        const [students] = await db.query(`
            SELECT first_name, phone, rent, room_number
            FROM students
            WHERE due_day=?
...
Amount: ₹${student.rent || 5500}

Your rent is due today.

Thank you,
Rahma Pearl Girls PG
`;
            ...
            try {
                ...
                console.log("Cron WhatsApp failed:", student.phone);
        ...
        if (students.length > 0) {
            ...
            await createNotification(
                `${students.length} rent reminders sent today.`
            );
        ...
        console.log("Daily Rent Reminder Cron Error:", err);
...
cron.schedule("0 0 1 * *", async () => {
    ...
    try {
        ...
        await createNotification(
            `Monthly rent generated for ${students.length} students.`
        );
        ...
        console.log("Monthly Rent Cron Error:", err);
...
Your rent payment is still pending.
...
`;
            ...
            try {
                ...
                console.log("Pending payment WhatsApp failed:", payment.phone);
        ...
        if (payments.length > 0) {
            ...
            await createNotification(
                `${payments.length} pending payment alerts sent.`
            );
        ...
        console.log("Pending Payment Cron Error:", err);
